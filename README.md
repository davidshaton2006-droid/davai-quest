...
Приложение "Здоровье" на iPhone не отдаёт данные напрямую — их выгружает **Команды**
(Shortcuts) через HTTP-запрос к Supabase REST API. Политики RLS требуют `auth.uid()` —
простой POST с одним `anon`-ключом без входа в аккаунт база не примет. Поэтому автоматизация
сначала логинится под твоим аккаунтом (email+пароль из приложения) и получает токен, и заодно
из того же ответа — свой `UID` (искать в Supabase Dashboard не нужно, он приходит вместе с
токеном).

**Шаг 0 — вход и получение UID** (в начале каждой автоматизации, повторяется в обеих):
1. Действие **«Получить содержимое URL»**:
   - URL: `https://khgquzlfglgtzknkoymg.supabase.co/auth/v1/token?grant_type=password`
   - Метод: `POST`
   - Заголовки: `apikey` = Publishable key, `Content-Type` = `application/json`
   - Тело запроса (Request Body → JSON): `email` = твой email в приложении, `password` = твой пароль
2. Действие **«Получить значение словаря»** на результате шага 1, ключ `access_token` → сохранить в переменную `AccessToken`.
3. Ещё раз **«Получить значение словаря»** на том же результате, ключ `user` → это вложенный словарь.
4. Ещё одно **«Получить значение словаря»** уже на результате шага 3, ключ `id` → сохранить в переменную `UID`.

**Automation 1 — сон** (триггер по времени, например 8:00):
1. Шаг 0 → получить `AccessToken` и `UID`.
2. **«Найти данные о здоровье»** → категория «Сон», период «Последняя ночь» → округлить часы сна (действие «Округлить число»).
3. **«Текст»** — собрать JSON-тело:
   `{"user_key":"UID","log_date":"ДАТА","hours":ЧАСЫ,"source":"apple_health"}`
   (дату взять действием «Форматировать дату», формат `ГГГГ-ММ-ДД`; `UID` и `ЧАСЫ` — вставить переменными из предыдущих шагов, через кнопку выбора переменной, не печатать текстом).
4. **«Получить содержимое URL»**:
   - URL: `https://khgquzlfglgtzknkoymg.supabase.co/rest/v1/sleep_logs`
   - Метод: `POST`
   - Заголовки: `apikey` = Publishable key, `Authorization` = `Bearer AccessToken` (переменная из шага 0), `Content-Type` = `application/json`, `Prefer` = `resolution=merge-duplicates`
   - Тело запроса: текст из шага 3.

**Automation 2 — активность** (шаги / активные минуты / часы стойки / пульс в покое):
То же самое, но:
   - Категория в «Найти данные о здоровье» — своя под каждую метрику.
   - URL: `https://khgquzlfglgtzknkoymg.supabase.co/rest/v1/activity_logs`
   - Тело: `{"user_key":"UID","log_date":"ДАТА","metric":"steps","value":ЗНАЧЕНИЕ,"source":"apple_health"}`
     (`metric` → `steps` | `active_minutes` | `stand_hours` | `resting_hr` — под каждую метрику своя копия автоматизации)

В настройках автоматизации выключить «Спрашивать перед запуском», чтобы всё летало в базу без
подтверждений. Токен из шага 0 живёт около часа — получаем заново при каждом запуске, хранить
не нужно.

**Важно про ключи:** Publishable ключ безопасен внутри Shortcuts только в паре с токеном
пользователя из шага 0. Ключ Secret (`service_role`) нигде не используется — он обходит RLS и
даёт полный доступ к чужим данным при утечке.
